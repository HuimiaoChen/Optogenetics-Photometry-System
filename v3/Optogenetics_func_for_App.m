...
function Optogenetics_func_for_App(app)
    ...
    stage1_switch = app.Stage1Switch.Value; % 'On' or 'Off', char
    ...
    %% signal settings
    % note: red uses the digital Output 0; blue uses the digital Output 1.

    High_V = 3;

    % signalData for pre-period
    pre_signalData = [High_V * ones(1, 500); High_V * ones(1, 500)]; % at least 500 scans required by NiDAQ
    ...
    for j = 1:stage_number 
        if switch_list(j) == "On"
            ...
            sti_signalData_red = [High_V * ones(1, t_pulse_red), zeros(1, t_zero_red)];
            sti_signalData_blue = [High_V * ones(1, t_pulse_blue), zeros(1, t_zero_blue)];
